...
# Usage:
#   scripts/experiment_18x_mount.sh --mode ich-safe|unlockcd|unlockcd-R|unlockcd-chain-sep [--boot ich|spiro|none] [--no-reboot] [--dry-run]
...
usage() {
    echo "usage: $0 --mode ich-safe|unlockcd|unlockcd-R|unlockcd-chain-sep [--boot ich|spiro|none] [--no-reboot] [--dry-run]"
}
while [ "$#" -gt 0 ]; do
    case "$1" in
...
case "$BOOT" in ich|spiro|none) ;; *) die "invalid --boot: $BOOT";; esac
...
# --- preflight ---
# boot=none: device boots externally (checkm8/SSHRD), chain is irrelevant — skip chain checks
if [ "$BOOT" != none ]; then
...
if [ "$DRY" = 1 ]; then
    if [ "$BOOT" = none ]; then
        BOOT_PLAN="device already in SSH ramdisk (booted externally); skipping boot phase"
    else
        BOOT_PLAN="check pwned DFU (irecovery -q | grep PWND:); boot chain via $BOOT ($([ "$BOOT" = ich ] && echo "$ICH_ROOT/boot.sh --sep" || echo "./spiro.sh boot $CHAIN_DIR"))"
    fi
    cat <<EOF
DRYRUN plan:
  1. $BOOT_PLAN
  2. start iproxy $SSH_PORT->22, wait SSH root@localhost:$SSH_PORT (alpine)
  3. scp $DEVICE_SCRIPT -> /var/root/ $( [ "$MODE" = unlockcd-chain-sep ] && echo "+ sep-firmware.img4 -> /var/root/sep-chain.img4" )
  4. ssh run: sh /var/root/mount_experiment_18.sh --$MODE (240s guard)
  5. parse VERDICT line$([ "$NO_REBOOT" = 1 ] || echo "; reboot device cleanly")
EOF
    exit 0
fi
...
# boot=none: device is already in an externally-booted SSH ramdisk, not in DFU — no pwned check
if [ "$BOOT" != none ]; then
...
# --- boot ---
if [ "$BOOT" = none ]; then
    log "boot=none: expecting device already in SSH ramdisk"
elif [ "$BOOT" = ich ]; then
